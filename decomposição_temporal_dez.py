...
#Separação da importação de gás natural do mês de dezembro
data_dez = data.iloc[:, 14]

#Visualização do gráfico da série temporal de dezembro
plt.figure(figsize = (10, 5))
plt.xlabel('Anos')
plt.ylabel('Quantidade de gás natural importado em dezembro (mil m³)')
plt.plot(data_dez)

#Decomposição da série temporal do mês de dezembro
decomposicao = seasonal_decompose(data_dez)
...
plt.figure(figsize = (10, 5))
plt.subplot(4, 1, 1)
plt.plot(data_dez, label = 'Dezembro')
plt.legend(loc = 'best')
