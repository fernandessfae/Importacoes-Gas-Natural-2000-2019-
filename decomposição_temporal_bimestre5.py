...
data['BIMESTRE2'] = data[['SET', 'OUT']].sum(axis=1)

#Seleção da coluna bimestral
b5 = data.iloc[:, 16]

#Visualização do gráfico da série temporal do 5º bimestre
plt.figure(figsize = (10, 5))
plt.xlabel('Anos')
plt.ylabel('Quantidade de gás natural importado no 5º bimestre (mil m³)')
plt.plot(b5)

#Decomposição da série temporal do 5º bimestre
decomposicao = seasonal_decompose(b5)
