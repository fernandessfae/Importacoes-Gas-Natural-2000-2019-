...
#Separação da importação de gás natural do mês de agosto
data_ago = data.iloc[:, 10]

#Visualização do gráfico da série temporal de agosto
plt.figure(figsize = (10, 5))
plt.xlabel('Anos')
plt.ylabel('Quantidade de gás natural importado em agosto (mil m³)')
plt.plot(data_ago)

#Decomposição da série temporal do mês de agosto
decomposicao = seasonal_decompose(data_ago)
...
plt.figure(figsize = (10, 5))
plt.subplot(4, 1, 1)
plt.plot(data_ago, label = 'Agosto')
plt.legend(loc = 'best')
