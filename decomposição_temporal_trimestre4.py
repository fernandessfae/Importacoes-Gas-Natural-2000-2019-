...
#Criação da coluna trimestral
data['TRIMESTRE3'] = data[['OUT', 'NOV', 'DEZ']].sum(axis=1)

#Seleção da coluna trimestral
t4 = data.iloc[:, 16]

#Visualização do gráfico da série temporal do 4º trimestre
plt.figure(figsize = (10, 5))
plt.xlabel('Anos')
plt.ylabel('Quantidade de gás natural importado no 4º trimestre (mil m³)')
plt.plot(t4)

#Decomposição da série temporal do 4º trimestre
decomposicao = seasonal_decompose(t4)
...
plt.figure(figsize = (10, 5))
plt.subplot(4, 1, 1)
plt.plot(t4, label = '4º Trimestre')
plt.legend(loc = 'best')
