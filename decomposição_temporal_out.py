...
#Separação da importação de gás natural do mês de outubro
data_out = data.iloc[:, 12]

#Visualização do gráfico da série temporal de outubro
plt.figure(figsize = (10, 5))
plt.xlabel('Anos')
plt.ylabel('Quantidade de gás natural importado em outubro (mil m³)')
plt.plot(data_out)

#Decomposição da série temporal do mês de outubro
decomposicao = seasonal_decompose(data_out)
...
plt.figure(figsize = (10, 5))
plt.subplot(4, 1, 1)
plt.plot(data_out, label = 'Outubro')
plt.legend(loc = 'best')
