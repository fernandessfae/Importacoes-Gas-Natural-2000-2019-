...
#Separação da importação de gás natural do mês de fevereiro
data_fev = data.iloc[:, 4]

#Visualização do gráfico da série temporal de fevereiro
plt.figure(figsize = (10, 5))
plt.xlabel('Anos')
plt.ylabel('Quantidade de gás natural importado em fevereiro (mil m³)')
plt.plot(data_fev)

#Decomposição da série temporal do mês de fevereiro
decomposicao = seasonal_decompose(data_fev)
...
plt.figure(figsize = (10, 5))
plt.subplot(4, 1, 1)
plt.plot(data_fev, label = 'Fevereiro')
plt.legend(loc = 'best')
