...
#Separação da importação de gás natural do mês de janeiro
data_jan = data.iloc[:, 3]

#Visualização do gráfico da série temporal de janeiro
plt.figure(figsize = (10, 5))
plt.xlabel('Anos')
plt.ylabel('Quantidade de gás natural importado em janeiro (mil m³)')
plt.plot(data_jan)

#Decomposição da série temporal do mês de janeiro
decomposicao = seasonal_decompose(data_jan)
...
plt.figure(figsize = (10, 5))
plt.subplot(4, 1, 1)
plt.plot(data_jan, label = 'Janeiro')
plt.legend(loc = 'best')
