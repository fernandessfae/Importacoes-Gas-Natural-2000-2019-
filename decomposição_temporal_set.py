...
#Separação da importação de gás natural do mês de setembro
data_set = data.iloc[:, 11]

#Visualização do gráfico da série temporal de setembro
plt.figure(figsize = (10, 5))
plt.xlabel('Anos')
plt.ylabel('Quantidade de gás natural importado em setembro (mil m³)')
plt.plot(data_set)

#Decomposição da série temporal do mês de setembro
decomposicao = seasonal_decompose(data_set)
...
plt.figure(figsize = (10, 5))
plt.subplot(4, 1, 1)
plt.plot(data_set, label = 'Setembro')
plt.legend(loc = 'best')
